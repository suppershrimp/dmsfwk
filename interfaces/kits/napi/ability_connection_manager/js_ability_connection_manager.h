...
private:
    ...
    static bool JsToSurfaceParam(const napi_env &env, const napi_value &jsValue, SurfaceParam &surfaceParam);

    static void ConnectThreadsafeFunctionCallback(napi_env env, napi_value js_callback, void* context, void* data);
    static napi_status CreateConnectThreadsafeFunction(napi_env env, napi_value js_func, napi_threadsafe_function* tsfn);
    ...
    static bool UnwrapConnectOptionParameters(napi_env env, napi_value parameters, ConnectOption &option);
