...
void DmsFreeInstallCallbackProxy::OnInstallFinished(int32_t resultCode)
{
    MessageParcel data;
    MessageParcel reply;
    ...
    int32_t error = Remote()->SendRequest(IDmsFreeInstallCallbackCmd::ON_FREE_INSTALL_DONE, data,
        reply, option);
    if (error != NO_ERROR) {
