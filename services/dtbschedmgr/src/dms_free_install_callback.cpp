...
DmsFreeInstallCallback::DmsFreeInstallCallback(int32_t taskId, const CallerInfo& callerInfo,
    const OHOS::AAFwk::Want& want)
    : taskId_(taskId), callerInfo_(callerInfo), want_(want)
{
...
void DmsFreeInstallCallback::OnInstallFinished(const OHOS::AAFwk::Want& want, int32_t requestCode, int32_t resultCode)
{
    ...
    DistributedSchedService::GetInstance().NotifyCompleteFreeInstall(info, taskId_, resultCode);
}
}  // namespace DistributedSchedule
}  // namespace OHOS
