...
void DistributedMissionFocusedListener::OnMissionDestroyed(int32_t missionId)
{
    ...
    if (!MultiUserManager::GetInstance().IsCallerForeground(callingUid)) {
        ...
        MultiUserManager::GetInstance().GetCurrentSendMgr()->NotifyMissionUnfocused(missionId,
            UnfocusedReason::DESTORY);
...
void DistributedMissionFocusedListener::OnMissionFocused(int32_t missionId)
{
    ...
    if (!MultiUserManager::GetInstance().IsCallerForeground(callingUid)) {
        ...
        MultiUserManager::GetInstance().GetCurrentSendMgr()->NotifyMissionFocused(missionId, FocusedReason::NORMAL);
    }
}
...
void DistributedMissionFocusedListener::OnMissionClosed(int32_t missionId)
{
    ...
    if (!MultiUserManager::GetInstance().IsCallerForeground(callingUid)) {
        ...
        MultiUserManager::GetInstance().GetCurrentSendMgr()->NotifyMissionUnfocused(missionId, UnfocusedReason::CLOSE);
    }
}
