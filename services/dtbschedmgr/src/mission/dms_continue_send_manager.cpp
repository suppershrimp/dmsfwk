...
int32_t DMSContinueSendMgr::DealFocusedBusiness(const int32_t missionId)
{
    ...
    focusedMissionAbilityName_[missionId] = abilityName;
...
int32_t DMSContinueSendMgr::DealUnfocusedBusiness(const int32_t missionId, UnfocusedReason reason)
{
    ...
    if (reason != UnfocusedReason::TIMEOUT) {
        ...
        focusedMissionAbilityName_.erase(missionId);
    }
...
int32_t DMSContinueSendMgr::GetAbilityNameByMissionId(const int32_t missionId, std::string& abilityName)
{
    ...
    std::lock_guard<std::mutex> focusedMissionAbilityNameMapLock(eventMutex_);
    auto iterItem = focusedMissionAbilityName_.find(missionId);
    if (iterItem != focusedMissionAbilityName_.end()) {
        abilityName = iterItem->second;
        HILOGI("get missionId end, missionId: %{public}d", missionId);
        return ERR_OK;
    }
    HILOGW("get iterItem failed from focusedMissionAbilityName_, try screenOffHandler_");
    ...
    const AAFwk::ContinueState &state)
{
    ...
    ret = SetStateSendEvent(accessTokenId, continueTypeId, state);
    if (ret != ERR_OK) {
