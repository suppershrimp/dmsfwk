    ...
    uint16_t bundleNameId, uint8_t continueTypeId, const int32_t state, const int32_t retry)
{
    ...
    HILOGI("continueType is %{public}s", continueType.c_str());
    std::vector<sptr<IRemoteObject>> objs = iterItem->second;
    for (auto iter : objs) {
        NotifyRecvBroadcast(iter, senderNetworkId, bundleName, continueType, state);
    }
...
void DMSContinueRecvMgr::NotifyRecvBroadcast(const sptr<IRemoteObject>& obj,
    const std::string& networkId, const std::string& bundleName, const std::string& continueType, const int32_t state)
{
