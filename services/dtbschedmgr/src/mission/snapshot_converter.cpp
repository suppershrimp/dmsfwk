...
int32_t SnapshotConverter::ConvertToSnapshot(AAFwk::MissionSnapshot& missionSnapshot, Snapshot& snapshot)
{
    ...
    std::unique_ptr<Rect> contentInsets(new Rect(0, 0, 0, 0));
...
int32_t SnapshotConverter::ConvertToSnapshot(AAFwk::MissionSnapshot& missionSnapshot, std::unique_ptr<Snapshot>& snapshot)
{
    snapshot->version_ = DMS_VERSION;
    snapshot->orientation_ = 0;
    std::unique_ptr<Rect> contentInsets(new Rect(0, 0, 0, 0));
...
int32_t SnapshotConverter::ConvertToMissionSnapshot(Snapshot& snapshot, std::unique_ptr<AAFwk::MissionSnapshot>& missionSnapshot)
{
