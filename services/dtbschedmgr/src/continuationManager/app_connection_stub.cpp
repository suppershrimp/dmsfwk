...
{
    HILOGD("called.");
    DistributedSchedService::GetInstance().HandleStartDeviceManager(remoteObject, token_, continuationExtraParams_);
    return;
}
...
{
    HILOGD("called.");
    DistributedSchedService::GetInstance().HandleStartDeviceManager(nullptr, token_, continuationExtraParams_);
    return;
}
} // namespace DistributedSchedule
} // namespace OHOS