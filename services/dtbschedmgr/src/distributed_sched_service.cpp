    ...
    int32_t callerUid, int32_t requestCode, uint32_t accessToken, const sptr<IRemoteObject>& callback)
{
    ...
    if (dmsCallbackSession_->PushCallback(sessionId, callback, deviceId, isContinue, want) != ERR_OK) {
...
int32_t DistributedSchedService::NotifyFreeInstall(int64_t sessionId, int32_t resultCode)
{
    ...
    if (dmsCallbackSession_->PopCallback(sessionId, item) != ERR_OK) {
        HILOGE("Pop callback failed!");
        return NO_CONNECT_CALLBACK_ERR;
    }
