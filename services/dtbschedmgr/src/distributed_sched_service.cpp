...
bool DistributedSchedService::IsContinuationModeValid(ContinuationMode continuationMode)
{
...
bool DistributedSchedService::IsConnectStatusValid(DeviceConnectStatus deviceConnectStatus)
{
...
void DistributedSchedService::ScheduleStartDeviceManager(const sptr<IRemoteObject>& appProxy, int32_t token,
...
void DistributedSchedService::HandleStartDeviceManager(int32_t token,
    const std::shared_ptr<ContinuationExtraParams>& continuationExtraParams)
{
    if (continuationHandler_ == nullptr) {
        HILOGE("continuationHandler_ is nullptr");
