    ...
    int32_t callerUid, int32_t status, uint32_t accessToken)
{
    ...
    bool flag = dschedContinuation_->IsFreeInstall(sessionId);
    if (flag) {
    ...
    int32_t callerUid, int32_t requestCode, uint32_t accessToken, const sptr<IRemoteObject>& callback)
{
    ...
    int64_t sessionId = dmsCallbackSession_->GenerateSessionId();
    ...
    if (result != ERR_OK) {
        ...
        NotifyFreeInstall(sessionId, result);
    }
    return result;
}

int32_t DistributedSchedService::StartFreeInstallFromRemote(const FreeInstallInfo& info, int64_t sessionId)
{
...
int32_t DistributedSchedService::NotifyCompleteFreeInstall(const std::string& deviceId, int64_t sessionId,
    int32_t resultCode)
{
...
int32_t DistributedSchedService::NotifyCompleteFreeInstallFromRemote(int64_t sessionId, int32_t resultCode)
{
    HILOGI("NotifyCompleteFreeInstallFromRemote begin sessionId = %{public}" PRId64 ", resultCode = %{public}d",
    ...
    int64_t continuationSessionId = dmsCallbackSession_->GetContinuaionSessionId(sessionId);
...
int32_t DistributedSchedService::NotifyFreeInstall(int64_t sessionId, int32_t resultCode)
{
