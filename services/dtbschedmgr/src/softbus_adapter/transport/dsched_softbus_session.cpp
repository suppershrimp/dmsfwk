...
{
    refCnt_++;
    HILOGI("session %d current ref cnt: %d", sessionId_, refCnt_);
}

void DSchedSoftbusSession::OnDisconnect()
{
    refCnt_--;
    HILOGI("session %d current ref cnt: %d", sessionId_, refCnt_);
    if (refCnt <= 0) {
        HILOGI("session: %d not in use, clear resource", sessionId_);
        DSchedTransportSoftbusAdapter::GetInstance()->OnShutdown(sessionId_);
    }
}

int32_t DSchedSoftbusSession::OnBytesReceived(std::shared_ptr<DSchedDataBuffer> dataBuffer)
{
    HILOGD("called");
    PackRecvData(buffer);
    return ERR_OK;
...
void DSchedSoftbusSession::PackRecvData(std::shared_ptr<DSchedDataBuffer> dataBuffer)
{
    ...
    if (buffer->Size() < BINARY_HEADER_FRAG_LEN) {
        ...
        HILOGE("pack recv data error, size: %" PRIu64", session id: %d", bufferSize, sessionId);
        return;
    }
    ...
    if (buffer->Size() != (headerPara.dataLen + BINARY_HEADER_FRAG_LEN) || headerPara.dataLen > headerPara.totalLen ||
        ...
        HILOGE("pack recv data failed, size: %" PRIu64", dataLen: %d, totalLen: %d session id:"
            " %d", bufferSize, headerPara.dataLen, headerPara.totalLen, sessionId);
        return;
    }
    bufferSize = static_cast<uint64_t>(buffer->Size());
...
void DSchedSoftbusSession::GetFragDataHeader(uint8_t *ptrPacket, SessionDataHeader& headerPara)
{
    ...
    while (i < BINARY_HEADER_FRAG_LEN) {
        i += ReadTlvToHeader(ptrPacket + i, headerPara);
    }
    return;
}
...
void DSchedSoftbusSession::AssembleNoFrag(std::shared_ptr<DSchedDataBuffer> buffer, SessionDataHeader& headerPara)
{
    if (headerPara.dataLen != headerPara.totalLen) {
        HILOGE("error, header lenth not match, dataLen: %d, totalLen: %d, session id: %d peerNetworkId: %s",
            ...
            DnetworkAdapter::AnonymizeNetworkId(peerNetworkId_).c_str());
        return;
    }
    std::shared_ptr<DSchedDataBuffer> postData = std::make_shared<DataBuffer>(headerPara.dataLen);
    ...
    if (ret != ERR_OK) {
        HILOGE("memcpy_s failed, ret: %d, session id: %d peerNetworkId: %s",
            ret, sessionId_, DnetworkAdapter::AnonymizeNetworkId(peerNetworkId_).c_str());
        return;
    }
    DSchedTransportSoftbusAdapter::GetInstance()->OnDataReady(postData);
}
...
void DSchedSoftbusSession::AssembleFrag(std::shared_ptr<DSchedDataBuffer> buffer, SessionDataHeader& headerPara)
{
    ...
    if (headerPara.fragFlag == FRAG_END) {
        DSchedTransportSoftbusAdapter::GetInstance()->OnDataReady(packBuffer_);
        ResetAssembleFrag();
    }
}
...
int32_t DSchedSoftbusSession::CheckUnPackBuffer(SessionDataHeader& headerPara)
{
    ...
    if (nowSeq_ != headerPara.seqNum) {
        HILOGE("seq error, nowSeq: %d actualSeq: %d, session id: %d peerNetworkId: %s",
            nowSeq_, headerPara.seqNum, sessionId_, DnetworkAdapter::AnonymizeNetworkId(peerNetworkId_).c_str());
        return INVALID_PARAMETERS_ERR;
    }

    if (nowSubSeq_ + 1 != headerPara.subSeq) {
        HILOGE("subSeq error, nowSeq: %d actualSeq: %d, session id: %d peerNetworkId: %s",
            nowSubSeq_, headerPara.subSeq, sessionId_, DnetworkAdapter::AnonymizeNetworkId(peerNetworkId_).c_str());
        return INVALID_PARAMETERS_ERR;
    }
    ...
    if (totalLen_ < headerPara.dataLen + offset_) {
        HILOGE("data len error cap: %d size: %d, dataLen: %d, session id: %d peerNetworkId: %s", totalLen_,
            offset_, headerPara.dataLen, sessionId_, DnetworkAdapter::AnonymizeNetworkId(peerNetworkId_).c_str());
        return INVALID_PARAMETERS_ERR;
    }
    return ERR_OK;
}
...
int32_t DSchedSoftbusSession::UnPackSendData(std::shared_ptr<DSchedDataBuffer> buffer)
{
    ...
    SessionDataHeader headPara = { PROTOCOL_VERSION, FRAG_START, DATA_TYPE_CONTINUE, seq, totalLen, subSeq };
    // use static lenth for now
    if (buffer->Size() <= BINARY_DATA_PACKET_MAX_LEN) {
        headPara.fragFlag = FRAG_START_END;
        headPara.dataLen = buffer->Size();
        std::shared_ptr<DSchedDataBuffer> unpackData = std::make_shared<DSchedDataBuffer>(buffer->Size() +
        ...
        int32_t ret = memcpy_s(unpackData->Data() + BINARY_HEADER_FRAG_LEN,
            unpackData->Size() - BINARY_HEADER_FRAG_LEN, buffer->Data(), buffer->Size());
        if (ret != ERR_OK) {
            HILOGE("START_END memcpy_s failed, ret: %d, sess: %d", ret, sessionId_);
            return ret;
        }
        return DSchedTransportSoftbusAdapter::GetInstance()->SendBytes(unpackData);
    }
    uint32_t offset = 0;
    uint64_t bufferSize;
    while (totalLen > offset) {
        ...
        bufferSize = static_cast<uint64_t>(buffer->Size());
        HILOGD("size: %" PRIu64", dataLen: %d, totalLen: %d, nowTime: %" PRId64" start:",
            bufferSize, headPara.dataLen, headPara.totalLen, GetNowTimeStampUs());
        std::shared_ptr<DSchedDataBuffer> unpackData =
            std::make_shared<DSchedDataBuffer>(headPara.dataLen + BINARY_HEADER_FRAG_LEN);
        MakeFragDataHeader(headPara, unpackData->Data(), BINARY_HEADER_FRAG_LEN);
        int32_t ret = memcpy_s(unpackData->Data() + BINARY_HEADER_FRAG_LEN,
            unpackData->Size() - BINARY_HEADER_FRAG_LEN, buffer->Data() + offset, headPara.dataLen);
        if (ret != ERR_OK) {
            HILOGE("memcpy_s failed, ret: %d, sess: %d", ret, sessionId_);
            return ret;
        }
        ret = DSchedTransportSoftbusAdapter::GetInstance()->SendBytes(unpackData);
        if (ret != ERR_OK) {
            HILOGE("sendData failed, ret: %d, sess: %d", ret, sessionId_);
            return ret;
        }
        HILOGD("size: %" PRIu64", dataLen: %d, totalLen: %d, nowTime: %" PRId64" end:",
            bufferSize, headPara.dataLen, headPara.totalLen, GetNowTimeStampUs());
        headPara.subSeq++;
...
{
    uint32_t headerLen = sizeof(uint8_t) * (HEADER_TYPE_BYTE + HEADER_LEN_BYTE) * HEADER_TLV_NUM  +
    ...
    TlvItem<uint16_t> version = { TLV_TYPE_VERSION, sizeof(uint16_t), headPara.version };
    WriteTlvToBuffer(version, header[i], capacity);
    capacity -= sizeof(uint16_t);
    i += sizeof(uint16_t);

    TlvItem<uint8_t> flag = { TLV_TYPE_FRAG_FLAG, sizeof(uint8_t), headPara.fragFlag };
    WriteTlvToBuffer(fragFlag, header[i], capacity);
    capacity -= sizeof(uint8_t);
    i += sizeof(uint8_t);

    TlvItem<uint32_t> dataType = { TLV_TYPE_DATA_TYPE, sizeof(uint32_t), headPara.dataType };
    WriteTlvToBuffer(dataType, header[i], capacity);
    capacity -= sizeof(uint32_t);
    i += sizeof(uint32_t);

    TlvItem<uint32_t> seqNum = { TLV_TYPE_SEQ_NUM, sizeof(uint32_t), headPara.seqNum };
    WriteTlvToBuffer(seqNum, header[i], capacity);
    capacity -= sizeof(uint32_t);
    i += sizeof(uint32_t);

    TlvItem<uint32_t> totalLen = { TLV_TYPE_TOTAL_LEN, sizeof(uint32_t), headPara.totalLen };
    WriteTlvToBuffer(totalLen, header[i], capacity);
    capacity -= sizeof(uint32_t);
    i += sizeof(uint32_t);

    TlvItem<uint16_t> subSeq = { TLV_TYPE_SUB_SEQ, sizeof(uint16_t), headPara.subSeq };
    WriteTlvToBuffer(subSeq, header[i], capacity);
    capacity -= sizeof(uint16_t);
    i += sizeof(uint16_t);

    TlvItem<uint32_t> dataLen = { TLV_TYPE_DATA_LEN, sizeof(uint32_t), headPara.dataLen };
    WriteTlvToBuffer(dataLen, header[i], capacity);
}

bool DSchedSoftbusSession::WriteTlvToBuffer(TlvItem tlvItem, uint8_t *buffer,  uint32_t bufLen)
{
    uint32_t tlvLen = sizeof(tlvItem.type) + sizeof(tlvItem.len) + sizeof(tlvItem.value);
    if (bufLen < tlvLen) {
    ...
    if (!memcpy_s(buffer[i], bufLen - tlvItem.len, &tlvItem.value, sizeof(tlvItem.value))) {
        HILOGE("copy tlv value failed!")
    }
}
