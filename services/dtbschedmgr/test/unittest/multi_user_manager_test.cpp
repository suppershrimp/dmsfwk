...
/**
 * @tc.name: MultiUserManager_IsCallerForeground_001
 * @tc.desc: test IsCallerForeground
 * @tc.type: FUNC
 */
HWTEST_F(MultiUserManagerTest, MultiUserManager_IsCallerForeground_001, TestSize.Level3)
{
    DTEST_LOG << "MultiUserManager_IsCallerForeground_001 start" << std::endl;
    /**
    ...
    bool ret = MultiUserManager::GetInstance().IsCallerForeground(accountId);
    EXPECT_TRUE(ret);

    ret = MultiUserManager::GetInstance().IsCallerForeground(otherAccountId);
    EXPECT_FALSE(ret);
