...
const std::string TEST_INVALID_CONTINUE_TYPE = "invalidContinueType";
    ...
    AppExecFwk::BundleInfo &localBundleInfo)
{
    ...
    abilityInfo.continueType.push_back("continueType_ContinueQuickStart");
...
{
    DTEST_LOG << "DSchedContinueTest CompleteContinueInfoTest_001 begin" <<
    std::endl;
    ...
    EXPECT_TRUE(ret);
    DTEST_LOG << "DSchedContinueTest CompleteContinueInfoTest_001 end ret:" << ret <<
    std::endl;
}

/**
 * @tc.name: CompleteContinueInfoTest_002
 * @tc.desc: DSchedContinueInfoUtil
 * @tc.type: FUNC
 */
    HWTEST_F(DSchedContinueInfoUtilTest, CompleteContinueInfoTest_002, TestSize.Level0)
{
    DTEST_LOG << "DSchedContinueTest CompleteContinueInfoTest_001 begin" <<
    std::endl;
    DSchedContinueInfo continueInfo;
    ...
    // max retry time = false
    bool ret = DSchedContinueInfoUtil::GetInstance().CompleteContinueInfo(
            TEST_SOURCE_NETWORK_ID, TEST_BUNDLE_NAME, TEST_CONTINUE_TYPE, continueInfo, DBMS_RETRY_MAX_TIME);
    EXPECT_FALSE(ret);

    // invalid networkId = false
    ret = DSchedContinueInfoUtil::GetInstance().CompleteContinueInfo(
            TEST_INVALID_SOURCE_NETWORK_ID, TEST_BUNDLE_NAME, TEST_CONTINUE_TYPE, continueInfo);
    EXPECT_FALSE(ret);

    // no matched continueTypeId complete src = false
    ret = DSchedContinueInfoUtil::GetInstance().CompleteContinueInfo(
            TEST_SOURCE_NETWORK_ID, TEST_BUNDLE_NAME, TEST_INVALID_CONTINUE_TYPE, continueInfo);
    EXPECT_FALSE(ret);

    // no continue bundleNames = false
    ret = DSchedContinueInfoUtil::GetInstance().CompleteContinueInfo(
            TEST_SOURCE_NETWORK_ID, TEST_EMPTY_CONTINUE_BUNDLE_BUNDLE_NAME, TEST_CONTINUE_TYPE, continueInfo);
    EXPECT_TRUE(ret);

    // sink not exist src bundleName = false
    ret = DSchedContinueInfoUtil::GetInstance().CompleteContinueInfo(
            TEST_SOURCE_NETWORK_ID, TEST_SINK_NOT_EXIST_BUNDLE_NAME_NAME, TEST_CONTINUE_TYPE, continueInfo);
    EXPECT_FALSE(ret);

    //
    ret = DSchedContinueInfoUtil::GetInstance().CompleteContinueInfo(
            TEST_SOURCE_NETWORK_ID, TEST_BUNDLE_NAME, TEST_CONTINUE_TYPE, continueInfo);
    EXPECT_TRUE(ret);
    DTEST_LOG << "DSchedContinueTest CompleteContinueInfoTest_002 end ret:" << ret <<
    std::endl;
}
}
}
