...
public:
    ...
    std::share_ptr<DMSContinueSendMgr> GetCurrentSendMgr();
    std::share_ptr<DMSContinueRecvMgr> GetCurrentRecvMgr();
    std::share_ptr<DMSContinueSendMgr> GetSendMgrByCallingUid(int32_t callingUid);
    std::share_ptr<DMSContinueRecvMgr> GetRecvMgrByCallingUid(int32_t callingUid);
...
private:
    ...
    std::map<int32_t, std::share_ptr<DMSContinueSendMgr>> sendMgrMap_;
    std::map<int32_t, std::share_ptr<DMSContinueRecvMgr>> recvMgrMap_;
