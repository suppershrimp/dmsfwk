 ...
 * @tc.desc: test OnRemoteRequest to start GetDistributedComponentListInner but func is nullptr
 * @tc.type: FUNC
 * @tc.require: I5NOA1
 */
HWTEST_F(DistributedAbilityManagerServiceTest, OnRemoteRequest_002, TestSize.Level3)
{
    ...
    MessageOption option;
    data.WriteInterfaceToken(DMS_STUB_INTERFACE_TOKEN);
    dtbabilitymgrService->distributedFuncMap_[GET_DISTRIBUTED_COMPONENT_LIST_REQUEST_CODE] = nullptr;
    int32_t result = dtbabilitymgrService->OnRemoteRequest(
        GET_DISTRIBUTED_COMPONENT_LIST_REQUEST_CODE, data, reply, option);
...
HWTEST_F(DistributedAbilityManagerServiceTest, OnRemoteRequest_005, TestSize.Level3)
{
    ...
    MessageOption option;
    data.WriteInterfaceToken(DMS_STUB_INTERFACE_TOKEN);
    dtbabilitymgrService->funcsMap_[UPDATE_CONNECT_STATUS] = nullptr;
    int32_t result = dtbabilitymgrService->OnRemoteRequest(UPDATE_CONNECT_STATUS, data, reply, option);
    EXPECT_EQ(ERR_NULL_OBJECT, result);
    delete dtbabilitymgrService;
 ...
 * @tc.require: I5NOA1
 */
HWTEST_F(DistributedAbilityManagerServiceTest, OnRemoteRequest_006, TestSize.Level3)
{
    DTEST_LOG << "DistributedAbilityManagerServiceTest OnRemoteRequest_006 start" << std::endl;
    DistributedAbilityManagerService  *dtbabilitymgrService =
        new DistributedAbilityManagerService(DISTRIBUTED_SCHED_SA_ID, true);
    if (dtbabilitymgrService == nullptr) {
        DTEST_LOG << "dtbabilitymgrService is nullptr" << std::endl;
    ...
    EXPECT_NE(ERR_NONE, result);
    delete dtbabilitymgrService;
    DTEST_LOG << "DistributedAbilityManagerServiceTest OnRemoteRequest_006 end" << std::endl;
}
}
}
