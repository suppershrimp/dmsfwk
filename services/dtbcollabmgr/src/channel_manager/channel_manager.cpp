...
#define CHECK_SOCKET_AND_CHANNEL(socketId, channelId)                             \
    do {                                                                          \
        if ((socketId) <= 0) {                                                    \
...
#define CHECK_DATA_NULL(socketId, data, errorHandler)                          \
    ...
    static void OnStreamRecv(int32_t socket, const StreamData* data, const StreamData* ext, const StreamFrameInfo* param)
    {
    ...
    int32_t ChannelManager::Init(const std::string& ownerName)
    {
        ...
        ret = Listen(socketServerId, g_low_qosInfo, g_low_QosTV_Param_Index, &channelManagerListener);
        if (ret != ERR_OK) {
    ...
    int32_t ChannelManager::PostTask(const AppExecFwk::InnerEvent::Callback& callback, const AppExecFwk::EventQueue::Priority priority)
    {
    ...
    int32_t ChannelManager::GenerateNextId(const ChannelDataType dataType)
    {
        ...
        if (idQueue.empty()) {
            ...
            if (channelId - CHANNEL_ID_GAP * (static_cast<int32_t>(dataType) + 1) >= CHANNEL_ID_GAP) {
                HILOGE("type %{public}d exceed max channel", static_cast<int32_t>(dataType));
    ...
    {
        int32_t clientSocketId = CreateClientSocket(info.channelName, info.peerInfo.peerName, info.peerInfo.networkId, dataType);
        if (clientSocketId <= 0) {
    ...
    int32_t ChannelManager::CreateClientSocket(const std::string& channelName, const std::string& peerName, const std::string& peerNetworkId, const ChannelDataType dataType)
    {
        HILOGI("start");
    ...
    int32_t ChannelManager::RegisterChannelListener(const int32_t channelId, const std::shared_ptr<IChannelListener> listener)
    {
    ...
    int32_t ChannelManager::DoBindSockets(const std::vector<int32_t>& socketIds, const ChannelDataType dataType)
    {
    ...
    inline std::optional<std::string> ChannelManager::GetChannelNameFromSocket(const std::string& socketName)
    {
    ...
    inline std::optional<ChannelDataType> ChannelManager::GetChannelDataTypeFromName(const std::string& channelName)
    {
    ...
    void ChannelManager::NotifyListeners(const int32_t channelId, Func listenerFunc, const AppExecFwk::EventQueue::Priority priority, Args&&... args)
    {
    ...
    {
        int32_t channelId = 0;
        CHECK_SOCKET_AND_CHANNEL(socketId, channelId);
        DoErrorCallback(channelId, errorCode);
    }

    void ChannelManager::DoErrorCallback(const int32_t channelId, const int32_t errorCode)
    {
        NotifyListeners(channelId, &IChannelListener::OnError, AppExecFwk::EventQueue::Priority::IMMEDIATE, errorCode);
    }

    void ChannelManager::DoConnectCallback(const int32_t channelId)
    {
        NotifyListeners(channelId, &IChannelListener::OnConnect, AppExecFwk::EventQueue::Priority::IMMEDIATE);
    }
    ...
    {
        NotifyListeners(channelId, &IChannelListener::OnDisConnect, AppExecFwk::EventQueue::Priority::IMMEDIATE);
    }
    ...
    inline int32_t ChannelManager::DoSendBytes(const int32_t channelId, const std::shared_ptr<AVTransDataBuffer>& data)
    {
        HILOGI("start to send bytes");
    ...
    int32_t ChannelManager::DoSendMessage(const int32_t channelId, const std::shared_ptr<AVTransDataBuffer>& data)
    {
        HILOGI("start to send message");
        return DoSendData(channelId, &DataSenderReceiver::SendMessageData, data);
    }

    void ChannelManager::OnBytesReceived(const int32_t socketId, const void* data, const uint32_t dataLen)
    {
        int32_t channelId = 0;
        CHECK_SOCKET_AND_CHANNEL(socketId, channelId);
    ...
    std::shared_ptr<AVTransDataBuffer> ChannelManager::ProcessRecvData(const int32_t channelId, const int32_t socketId, const void* data, const uint32_t dataLen)
    {
        std::shared_lock<std::shared_mutex> readLock(channelMutex_);
    ...
    void ChannelManager::DoBytesReceiveCallback(const int32_t channelId, const std::shared_ptr<AVTransDataBuffer>& buffer)
    {
        NotifyListeners(channelId, &IChannelListener::OnBytes, AppExecFwk::EventQueue::Priority::LOW, buffer);
    }
    ...
    void ChannelManager::OnMessageReceived(const int32_t socketId, const void* data, const uint32_t dataLen)
    {
        ...
        int32_t ret = memcpy_s(buffer->Data(), buffer->Size(), data, dataLen);
        if (ret != ERR_OK) {
            HILOGE("pack recv data failed");
    ...
    void ChannelManager::DoMessageReceiveCallback(const int32_t channelId, const std::shared_ptr<AVTransDataBuffer>& buffer)
    {
        NotifyListeners(channelId, &IChannelListener::OnMessage, AppExecFwk::EventQueue::Priority::HIGH, buffer);
    }

    void ChannelManager::OnStreamReceived(const int32_t socketId, const StreamData* data, const StreamData* ext, const StreamFrameInfo* param)
    {
        int32_t channelId = 0;
        CHECK_SOCKET_AND_CHANNEL(socketId, channelId);
        CHECK_DATA_NULL(socketId, data, OnError);
    ...
    {
        NotifyListeners(channelId, &IChannelListener::OnStream, AppExecFwk::EventQueue::Priority::LOW, data);
    }
}
}
