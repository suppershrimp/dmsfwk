...
namespace DistributedCollab {
    ...
    inline uint32_t MessageDataHeader::WriteVersion(uint8_t* header, const uint32_t bufLen)
    {
    ...
    inline uint32_t MessageDataHeader::WriteDataType(uint8_t* header, const uint32_t bufLen)
    {
    ...
    inline uint32_t MessageDataHeader::WriteTotalLen(uint8_t* header, const uint32_t bufLen)
    {
    ...
    inline int32_t MessageDataHeader::WriteTlvItemToBuffer(const MessageTlvItem& tlvItem, uint8_t* header, const uint32_t bufLen)
    {
    ...
    std::optional<MessageDataHeader> MessageDataHeader::Deserialize(const uint8_t* buffer, const size_t bufLen)
    {
        ...
        // auto versionIt = HEADER_LEN_VERSION_MAP.find(messageHeader.version_);
        // if (versionIt == HEADER_LEN_VERSION_MAP.end()) {
        //     HILOGW("recv data version higher, %{public}d", messageHeader.version_);
...
inline int32_t MessageDataHeader::ReadTlvItemFromBuffer(MessageTlvItem& tlvItem, MessageDataHeader& messageHeader,
    ...
    inline uint16_t MessageDataHeader::ReadUint16(const uint8_t* header)
    {
        return (header[0] << DSCHED_SHIFT_8) | header[1];
    }

    inline int32_t MessageDataHeader::ReadVersionFromBuffer(MessageDataHeader& messageHeader, const uint8_t* header, const uint32_t len)
    {
    ...
    inline int32_t MessageDataHeader::ReadDataTypeFromBuffer(MessageDataHeader& messageHeader, const uint8_t* header, const uint32_t len)
    {
    ...
    inline int32_t MessageDataHeader::ReadTotalLenFromBuffer(MessageDataHeader& messageHeader, const uint8_t* header, const uint32_t len)
    {
