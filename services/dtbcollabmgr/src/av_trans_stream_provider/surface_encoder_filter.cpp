...
namespace DistributedCollab {
    ...
    static Media::Pipeline::AutoRegisterFilter<SurfaceEncoderFilter> g_registerSurfaceEncoderFilter("builtin.dtbcollab.videoencoder",
        ...
        const std::shared_ptr<FilterCallback>& callback)
    {
        ...
        if (!codecAdapter_ || isUpdateCodecNeeded_) {
            ...
            if (ret == Status::OK) {
                std::shared_ptr<EncoderAdapterCallback> encoderAdapterCallback = std::make_shared<SurfaceEncoderAdapterCallback>(shared_from_this());
    ...
    void SurfaceEncoderFilter::SetParameter(const std::shared_ptr<Meta>& parameter)
    {
        ...
        if (parameter->Find(Tag::MEDIA_END_OF_STREAM) != parameter->end() && parameter->Get<Tag::MEDIA_END_OF_STREAM>(isEos) && parameter->Get<Tag::USER_FRAME_PTS>(eosPts)) {
            if (isEos) {
