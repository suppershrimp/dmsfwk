...
#define SAFE_MEMORY_READ_NO_RETRUN(dest, destSize, src, srcSize, headerJson)          \
    ...
    void AVReceiverFilter::Process()
    {
        ...
        while (isRunning_) {
            ...
            if (data->GetStreamDataExt().flag_ == AvCodecBufferFlag::AVCODEC_BUFFER_FLAG_PIXEL_MAP) {
                auto pixelMap = GetPixelMap(data);
                if (eventHandler_ == nullptr) {
                    continue;
                }
                auto func = [pixelMap, this]() {
                    if (auto ptr = engineListener_.lock()) {
