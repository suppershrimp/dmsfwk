...
namespace DistributedCollab {
    ...
    std::shared_ptr<AVSenderEngine> InitSender()
    {
        ...
        std::shared_ptr<AVSenderEngine> engine = std::make_shared<AVSenderEngine>(appUid, appPid, bundleName, instanceId);
    ...
    int32_t ConfigEngineParam(std::shared_ptr<T>& engine)
    {
        ...
        static_assert(std::is_member_function_pointer<decltype(&T::SetVideoSource)>::value, "T must have SetVideoSource method");
    ...
    sptr<SurfaceBuffer> GetInputBuffer(const sptr<Surface>& surface)
    {
        ...
        case VideoSourceType::YUVI420:
            config.format = PixelFormat::PIXEL_FMT_YCBCR_420_P;
            break;
        ...
        case VideoSourceType::RGBA:
            config.format = PixelFormat::PIXEL_FMT_RGBA_8888;
            break;
        default:
    ...
    public:
        ...
        int32_t PackPixelMap(const std::shared_ptr<Media::PixelMap>& pixelMap, std::unique_ptr<AVTransDataBuffer>& buffer)
        {
    ...
    void ReadJpegToBuffer(std::unique_ptr<Media::PixelMap>& pixelMap)
    {
        ...
        uint32_t err = ERR_OK;
        std::unique_ptr<Media::ImageSource> imageSource = Media::ImageSource::CreateImageSource(jpegReadFilePath, sourceOption, err);
        if (err != ERR_OK) {
    ...
    int32_t Execute(std::shared_ptr<AVSenderEngine>& senderEngine, std::shared_ptr<AVReceiverEngine>& recvEngine)
    {
        ...
        recvSurface->SetDefaultUsage(SurfaceBufferUsage::BUFFER_USAGE_MEM_MMZ_CACHE | SurfaceBufferUsage::BUFFER_USAGE_CPU_READ);
