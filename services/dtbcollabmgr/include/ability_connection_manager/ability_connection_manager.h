...
public:
    ...
    int32_t NotifyCollabResult(int32_t sessionId, bool isSuccess, const std::string& peerServerName);
    int32_t NotifyDisconnect(int32_t sessionId);

    int32_t SendMessage(int32_t sessionId, const std::string& msg);
    int32_t SendData(int32_t sessionId, std::shared_ptr<AVTransDataBuffer> buffer);
    int32_t SendImage(int32_t sessionId, std::shared_ptr<AVTransDataBuffer> &pixelMapPtr);

    int32_t CreateStream(int32_t sessionId, const StreamParams& param, int32_t& streamId);
    int32_t SetSurfaceId(int32_t streamId, const std::string& surfaceId, const SurfaceParam& param);
    int32_t GetSurfaceId(int32_t streamId, const SurfaceParam& param, std::string& surfaceId);
    int32_t updateSurfaceParam(int32_t streamId, SurfaceParam param);
