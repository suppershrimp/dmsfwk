...
namespace DistributedCollab {
    ...
    public:
        ...
        static constexpr uint32_t HEADER_LEN = 2 * sizeof(uint16_t) * HEADER_TLV_NUM + sizeof(uint16_t) * HEADER_UINT16_NUM + sizeof(uint32_t) * HEADER_UINT32_NUM;
    ...
    private:
        static constexpr uint32_t BASE_HEADER_LEN = 2 * sizeof(uint16_t) * HEADER_TLV_NUM + sizeof(uint16_t) * HEADER_UINT16_NUM + sizeof(uint32_t) * HEADER_UINT32_NUM;
    ...
    private:
        ...
        int32_t ReadTlvItemFromBuffer(MessageTlvItem& tlvItem, MessageDataHeader& sessionHeader, const uint8_t* header, const uint8_t* end);
