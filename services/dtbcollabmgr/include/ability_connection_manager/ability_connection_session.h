...
public:
    ...
    int32_t HandleCollabResult(bool isSuccess, const std::string& peerSocketName);
    int32_t HandleDisconnect();

    int32_t SendMessage(const std::string& msg, int32_t dataType = 0);
    int32_t SendData(std::shared_ptr<AVTransDataBuffer> buffer);
    int32_t SendImage(std::shared_ptr<AVTransDataBuffer> &pixelMapPtr);

    int32_t CreateStream(int32_t streamId, const StreamParams& param);
    int32_t DestroyStream(int32_t streamId);
    int32_t SetSurfaceId(const std::string& surfaceId, const SurfaceParam& param);
    int32_t GetSurfaceId(const SurfaceParam& param, std::string& surfaceId);
...
private:
    ...
    template <typename T>
    int32_t ConfigEngineParam(std::shared_ptr<T> &engine, const SurfaceParam& param);
    ...
    void ExeuteConnectCallback(bool result);
